...
{
	FILE* tp = fopen("temp.mp3", "w"); //Temp file opening in write mode for editing data
    	FILE* fp = fopen(argv[4], "r"); //Sample.mp3 file opening in read mode
	...
	header_check(header);  //header checking
    	...
    	for(int i = 0; i < 6; i++)  //Looping 6 times to check all tags
    	{
        	...
        	char tag[5];  //tag variable to the copy tag 
        	fread(tag, 4, 1, fp);
        	tag[4] = '\0';

        	int edittag = -1;
		//tags information
        	char mp3tag[6][5] = {"TIT2", "TPE1", "TALB", "TYER", "TCON", "COMM"}; 
        	char mp3arg[6][3] = {"-t", "-a", "-A", "-y", "-C", "-c"};  
        	char mp3tit[6][9] = {"Title", "Artist", "Album", "Year", "Contents", "Comments"}; 
        
		for(int j = 0; j < 6; j++)
        	{
            		if(strcmp(argv[2],mp3arg[j]) == 0)
            		{
                		edittag = j;  //edit tag variable to store index
                		break;
            		}
        	}
        ...
        rename("temp.mp3",argv[4]); //renaming the temporary file to original file
}
